...
# List to store cleaned, melted Jaccard matrices per genus
jaccard_melted_by_genus <- list()
for (genus in names(ASVtabs_by_genus)) {
  ...
  # Compute binary Jaccard distance (Sørensen)
  jacc_dist <- vegdist(asvtab_bin, method = "jaccard", binary = TRUE)
  jacc_mat <- as.matrix(jacc_dist)
  jacc_mat = 1 - jacc_mat
  # Melt the distance matrix
  jacc_melt <- melt(jacc_mat, varnames = c("Sample1", "Sample2"), value.name = "Jaccard_Similarity")
  ...
  jacc_melt_filtered <- jacc_melt %>%
    ...
    select(Sample1, Sample2, Jaccard_Similarity)
  # Add genus column
  jacc_melt_filtered$Genus <- genus
  # Save to list
  jaccard_melted_by_genus[[genus]] <- jacc_melt_filtered
}
...
for (genus in names(jaccard_melted_by_genus)) {
  ...
  aligned <- jacc_mat_melt_filtered %>%
    ...
    left_join(genus_df %>% select(pair_id, Jaccard_Similarity), by = "pair_id")# %>%
...
for (genus in names(jaccard_aligned_by_genus)) {
  ...
  jaccard_aligned_by_genus[[genus]] <- df
}


dfff =jaccard_aligned_by_genus$Prevotella

### Append metadata to it: 
all(
  jaccard_aligned_by_genus[[1]]$Sample1 == jacc_mat_melt_filtered$Sample1 &
    jaccard_aligned_by_genus[[1]]$Sample2 == jacc_mat_melt_filtered$Sample2
) # good 
...
df_check = jaccard_aligned_by_genus$Ruminococcus
...
mv = jaccard_aligned_by_genus$Marvinbryantia
