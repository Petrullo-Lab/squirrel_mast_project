...
asv_list <- readRDS("Jacc_aligned_by_genus_filtered_new.RDS")
...
model <- ordbetareg(
  Jacc_Similarity ~ 1 + mother_offspring*masting_factor + mother_offspring*same_season +
...
# Save as RDS as well
saveRDS(model, file = paste0("output_models/brms_model_", asv, ".rds"))

rrs2 <- avg_slopes(model, ndraws = 2000) 

saveRDS(rrs2, file = paste0("output_models/brms_predictions_", asv, ".rds"))
