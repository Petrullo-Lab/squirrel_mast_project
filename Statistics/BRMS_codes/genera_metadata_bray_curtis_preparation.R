...
# List to store cleaned, melted BC matrices per genus
BC_melted_by_genus <- list()
for (genus in names(ASVtabs_by_genus)) {
  ...
  # Compute binary BC distance (Sørensen)
  jacc_dist <- vegdist(asvtab_bin, method = "bray")
  jacc_mat <- as.matrix(jacc_dist)
  jacc_mat = 1 - jacc_mat
  # Melt the distance matrix
  jacc_melt <- melt(jacc_mat, varnames = c("Sample1", "Sample2"), value.name = "BC_Similarity")
  ...
  jacc_melt_filtered <- jacc_melt %>%
    ...
    select(Sample1, Sample2, BC_Similarity)
  # Add genus column
  jacc_melt_filtered$Genus <- genus
  # Save to list
  BC_melted_by_genus[[genus]] <- jacc_melt_filtered
}
...
BC_aligned_by_genus <- list()

for (genus in names(BC_melted_by_genus)) {
  
  genus_df <- BC_melted_by_genus[[genus]]
  ...
  aligned <- jacc_mat_melt_filtered %>%
    ...
    left_join(genus_df %>% select(pair_id, BC_Similarity), by = "pair_id")# %>%
...
for (genus in names(BC_aligned_by_genus)) {
  ...
  BC_aligned_by_genus[[genus]] <- df
}


dfff =BC_aligned_by_genus$Prevotella

### Append metadata to it: 
all(
  BC_aligned_by_genus[[1]]$Sample1 == jacc_mat_melt_filtered$Sample1 &
    BC_aligned_by_genus[[1]]$Sample2 == jacc_mat_melt_filtered$Sample2
) # good 
...
df_check = BC_aligned_by_genus$Ruminococcus
...
saveRDS(BC_aligned_by_genus,"/Users/gabri/Library/CloudStorage/OneDrive-UniversityofArizona/Squirrels/masting_project/BRMS_data_again/BC_aligned_by_genus_filtered_new.RDS")
nnnnnn = names(BC_aligned_by_genus)
...
#BC_aligned_by_genus = readRDS("/Users/gabri/Library/CloudStorage/OneDrive-UniversityofArizona/Squirrels/masting_project/BRMS_data_again/BC_aligned_by_genus_filtered_new.RDS")
mv = BC_aligned_by_genus$Marvinbryantia
mv$BC_Similarity