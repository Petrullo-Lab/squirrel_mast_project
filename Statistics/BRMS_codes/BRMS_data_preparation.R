...
#limit_15th = melted_diffs[melted_diffs$day_of_year1 < 200,]
#limit_15th = limit_15th[limit_15th$day_of_year2 < 200,]
...
melted_diffs_selected = melted_diffs %>%
  select(Sample1, Sample2, Jaccard_Similarity, BC_Similarity,squirrel1,squirrel2,masting,same_squirrel,same_mother,mother_offspring,
         year_dist,year_dist,dist_m,gr,julian_day_dist,read_dist,run,age_dist,same_sex, same_year, same_season)
melted_diffs_selected <- na.omit(melted_diffs_selected)
...
### Select until 15th august
limit_15th = melted_diffs[melted_diffs$day_of_year1 < 227,]
...
colnames(limit_15th)
limit_15th_selected = limit_15th %>%
  select(Sample1, Sample2, Jaccard_Similarity, BC_Similarity,squirrel1,squirrel2,masting,same_squirrel,same_mother,mother_offspring,
         year_dist,year_dist,dist_m,gr,julian_day_dist,read_dist,run,age_dist,same_sex, same_year, same_season)
limit_15th_selected <- na.omit(limit_15th_selected)
colnames(limit_15th_selected)

saveRDS(limit_15th_selected, "/Users/gabri/Library/CloudStorage/OneDrive-UniversityofArizona/Squirrels/masting_project/BRMS_data_again/Full_data_set__15th_Aug_BRMS.RDS")

## only same year comparisons 
limit_15th_selected_same_year = limit_15th_selected[limit_15th_selected$same_year == 1,]
...
limit_15th_selected_diff_year = limit_15th_selected[limit_15th_selected$same_year == 0,]
saveRDS(limit_15th_selected_same_year, "/Users/gabri/Library/CloudStorage/OneDrive-UniversityofArizona/Squirrels/masting_project/BRMS_data_again/Only_diff_year_15th_Aug_BRMS.RDS")
...
for(i in 1:ncol(limit_15th_selected[,which(colnames(limit_15th_selected)%in%scalecols)])){
  limit_15th_selected[,which(colnames(limit_15th_selected)%in%scalecols)][,i]<-range.use(limit_15th_selected[,which(colnames(limit_15th_selected)%in%scalecols)][,i],0,1)
}




hist(limit_15th_selected$same_year)

getwd()


ssel = readRDS("/Users/gabri/Library/CloudStorage/OneDrive-UniversityofArizona/Squirrels/analysis_data_oct_2025/16s/comb_ASV_tabs/code_BRMS/limit_15th_selected.RDS")
summary(ssel)
saveRDS(limit_15th_selected, "limit_15th_July_selected.RDS")
