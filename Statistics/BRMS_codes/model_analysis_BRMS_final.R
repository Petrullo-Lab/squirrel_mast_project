...
cor(post[, c("b_same_year","b_year_dist", "b_masting", "b_run", "b_mother_offspring:masting", "b_masting", "b_same_season")])
...
fe <- fe %>%
  ...
  mutate(term = recode(term,
                       ...
                       "same_sex" = "Same sex",
                       "mother_offspring:masting" = "Mother–offspring × Masting",
                       "mother_offspring:same_season" = "Mother-offspring x Same Season"
  ))
...
post <- model_bc %>%
  ...
  mutate(
    ...
    .variable = recode(.variable,
                       ...
                       "b_mother_offspring:masting" = "M/O × Masting",
...
# Plot distributions
bc2 = ggplot(post, aes(x = delta, fill = .variable)) +
  geom_density(fill = "grey70", color = "grey30", alpha = 0.7) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_wrap(~ .variable, scales = "free_y", nrow = 1) +
  theme_classic() +
  labs(x = "Difference in Bray-Curtis Similarity",
       y = "Posterior Density", title = "Bray-Curtis Similarity") +
  theme(legend.position = "none",
        strip.text = element_text(size = 9)) # Smaller x-axis label
...
# Get EMMs for the interaction
emm <- emmeans(model_jacc, ~ mother_offspring * masting)
# Compare masting levels (within each mother_offspring group)
pairs(emm, by = "masting")
pairs(emm, by = "masting",level = 0.99)  # For **
pairs(emm,by = "masting", level = 0.999) # For ***
emm_masting <- emmeans(model_jacc, ~ masting, type = "response")
pairs(emm_masting)
pairs(emm_masting, level = 0.99)  # For **
pairs(emm_masting, level = 0.999) # For ***
...
# Plot distributions
jacc2 = ggplot(post1, aes(x = delta, fill = .variable)) +
  geom_density(fill = "grey70", color = "grey30", alpha = 0.7) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  facet_wrap(~ .variable, scales = "free_y", nrow = 1) +
  theme_classic() +
  labs(x = "Difference in Jaccard Similarity",
       y = "Posterior Density", title = "Jaccard Similarity") +
  theme(legend.position = "none",
        strip.text = element_text(size = 9)) # Smaller x-axis label
